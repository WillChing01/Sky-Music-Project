...
// formerly get - rename to pop if this is staying
export const getHistory = (history) => {
    const nothingLeft = history.tracks[history.top] === null;
    if (nothingLeft) return null;
    const track = history.tracks[history.top];
    history.top = mod(history.current - 1, history.length);
    return track;
};
